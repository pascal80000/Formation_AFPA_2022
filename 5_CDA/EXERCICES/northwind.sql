...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 13 avr. 2023 à 16:30
-- Version du serveur : 10.6.12-MariaDB-0ubuntu0.22.10.1
...
-- Procédures
--
CREATE DEFINER=`admin`@`localhost` PROCEDURE `delai` (IN `date1` VARCHAR(20), `date2` VARCHAR(20))  BEGIN
            SELECT AVG(DATEDIFF(date1, date2))
            FROM orders;
        END$$

CREATE DEFINER=`admin`@`localhost` PROCEDURE `derniereCommande` (IN `company` VARCHAR(50))  BEGIN
...
(11078, 'VINET', NULL, NULL, NULL, NULL, NULL, '0.0000', NULL, NULL, NULL, NULL, NULL, 'France');
...
--
DELIMITER $$
CREATE TRIGGER `controle_pays_commande` BEFORE INSERT ON `order_details` FOR EACH ROW BEGIN
...
--
-- Doublure de structure pour la vue `v_before_insert`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `v_before_insert` (
...
--
-- Structure de la vue `v_before_insert`
--
DROP TABLE IF EXISTS `v_before_insert`;

CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`localhost` SQL SECURITY DEFINER VIEW `v_before_insert`  AS SELECT `o`.`ShipCountry` AS `ShipCountry`, `s`.`Country` AS `Country` FROM (((`suppliers` `s` join `products` `p` on(`s`.`SupplierID` = `p`.`ProductID`)) join `order_details` `od` on(`p`.`ProductID` = `od`.`ProductID`)) join `orders` `o` on(`od`.`OrderID` = `o`.`OrderID`)) ;
...
--
ALTER TABLE `orders`
  MODIFY `OrderID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11079;
